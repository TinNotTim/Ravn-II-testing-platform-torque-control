...
joint = 5
target_torque = 25.0  #assume this parameter is assignend by other higher controller 
...
if compensation_master:
    ...
    while(True):
        motor_pose = r2_tor_ctl.raven_state.mpose[13]
        motor_velocity = r2_tor_ctl.raven_state.mvel[13]

        if coulomb_compensation:
            #define the motor behavior based on velocity
            if motor_velocity > 0.2: #forward drive
                control_torque = control_torque + target_torque * coulomb_factor
        ...
        if viscous_compensation:
            control_torque += control_torque * viscous_factor

        #only send out control command if the value change
        if control_torque != target_torque and control_torque <= max_torque:
            cmd = np.zeros((16))
